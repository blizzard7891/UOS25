					...
					<tbody>
						...
						function do_fetch($s)
						{
							while($row = oci_fetch_array($s,OCI_RETURN_NULLS + OCI_ASSOC))
							{
								...
								foreach ($row as $item) 
								{
									if($item == $row['FLAG'] && trim($item)==="0")
										echo "<td>수입</td>.";
									elseif($item == $row['FLAG'] && trim($item)==="1")
										echo "<td>지출</td>";
									else
						...
						$query = "SELECT seq_num,change_date,description,flag,amount,remain_amount FROM MONEY_MANAGEMENT";
